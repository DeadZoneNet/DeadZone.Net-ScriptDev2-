...
{
    NPC_NASS                = 28521,
    NPC_NASS_CREDIT         = 28523,
    NPC_WITHERED_TROLL      = 28519,
    NPC_STEFAN_VADU         = 28518,
 
    SPELL_KICK_NASS         = 51866,
    SPELL_WITHERED_STRIKE   = 54345,
    QUEST_KICK_NASS         = 12630,
    GOSSIP_MENU_ID          = 13271,
    GOSSIP_ITEM_NASS        = -3631615,
};
 
struct MANGOS_DLL_DECL npc_nass_escort_startAI : public ScriptedAI
{
    npc_nass_escort_startAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        ...
        {
            if (Creature* pNassCredit = GetClosestCreatureWithEntry(m_creature, NPC_NASS_CREDIT, 15.0f))
            {
                ((Player*)pCaster)->KilledMonsterCredit(NPC_NASS_CREDIT, m_creature->GetGUID());
                m_creature->DealDamage(pNassCredit, pNassCredit->GetMaxHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NONE, NULL, false);
                pNassCredit->ForcedDespawn();
            }
        }
    }
...
bool QuestAccept_npc_stefan_vadu(Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_KICK_NASS)
        Creature* pNass = pCreature->SummonCreature(NPC_NASS, pPlayer->GetPositionX() + 3, pPlayer->GetPositionY() + 2, pPlayer->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 106000);
 
    return true;
}
 
bool GossipHello_npc_stefan_vadu(Player* pPlayer, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        pPlayer->PrepareQuestMenu(pCreature->GetGUID());
 
    if (pPlayer->GetQuestStatus(QUEST_KICK_NASS) == QUEST_STATUS_INCOMPLETE)
    {
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_NASS, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        pPlayer->SEND_GOSSIP_MENU(GOSSIP_MENU_ID, pCreature->GetGUID());
    }
...
bool GossipSelect_npc_stefan_vadu(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
    {
        if(!GetClosestCreatureWithEntry(pCreature, NPC_NASS, 25.0f))
            Creature* pNass = pCreature->SummonCreature(NPC_NASS, pPlayer->GetPositionX() + 3, pPlayer->GetPositionY() + 2, pPlayer->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 106000);
    }
    pPlayer->CLOSE_GOSSIP_MENU();
    return true;
...
struct MANGOS_DLL_DECL npc_withered_trollAI : public ScriptedAI
{
    npc_withered_trollAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
 
    uint64 m_uiTargetKcGUID;
    uint32 m_uiWitheredStrikeTimer;
 
    void Reset()
    {
        m_uiTargetKcGUID = 0;
        m_uiWitheredStrikeTimer = 2500;
    }

 
    void UpdateAI(uint32 const uiDiff)
    {
        if (m_uiWitheredStrikeTimer < uiDiff)
        {
            if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                DoCast(pTarget, SPELL_WITHERED_STRIKE);
            m_uiWitheredStrikeTimer = 3000;
        }
        else
            m_uiWitheredStrikeTimer -= uiDiff;
    ...
    {
        m_creature->SummonCreature(NPC_NASS_CREDIT, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 20000);
    }
};
 
CreatureAI* GetAI_npc_withered_troll(Creature* pCreature)
{
    return new npc_withered_trollAI(pCreature);
}

CreatureAI* GetAI_npc_nass_escort_start(Creature* pCreature)
{
    return new npc_nass_escort_startAI(pCreature);
}
...
void AddSC_zuldrak()
{
    ...
    pNewScript->Name = "npc_nass_escort";
    pNewScript->GetAI = &GetAI_npc_nass_escort_start;
    pNewScript->RegisterSelf();
 
    pNewScript = new Script;
    pNewScript->Name = "npc_stefan_vadu";
    pNewScript->pQuestAcceptNPC = &QuestAccept_npc_stefan_vadu;
    pNewScript->pGossipHello = &GossipHello_npc_stefan_vadu;
    pNewScript->pGossipSelect = &GossipSelect_npc_stefan_vadu;
    pNewScript->RegisterSelf();;
 
    pNewScript = new Script;
    pNewScript->Name = "npc_withered_troll";
    pNewScript->GetAI = &GetAI_npc_withered_troll;
    pNewScript->RegisterSelf();
